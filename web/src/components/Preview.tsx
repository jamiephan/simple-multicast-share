...
const textExt = new Set(['txt', 'md', 'json', 'toml', 'yaml', 'yml', 'csv', 'log', 'xml', 'html', 'css', 'js', 'ts', 'tsx', 'rs', 'go', 'py', 'sh'])
...
export function Preview({ entry, onClose, onChanged, onSaveImage }: PreviewProps) {
  ...
  const content = () => {
    ...
    switch (category) {
      ...
      case 'text': return <label className="text-editor"><span className="sr-only">File content</span><textarea value={text} spellCheck={false} onChange={(event) => setText(event.target.value)} /></label>
      case 'archive': return (
